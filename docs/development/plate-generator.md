...
- `cutout` — `switchType`, `stabilizerType`, `switchFilletRadius`, `stabilizerFilletRadius` (omitted when stab type is `none`), `kerf`, `merge`, `width`/`height` (only for `custom-rectangle`).
